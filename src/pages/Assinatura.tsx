...
const plans = [{
  ...
  features: ["Tudo do plano Premium", "Consultoria nutricional individual", "Planos adaptados a condições médicas", "Videochamadas com nutricionistas", "Lista de compras automática", "Acesso antecipado a novidades"],
  icon: Zap,
  popular: false,
...
const Assinatura = () => {
  ...
  return <div className="min-h-screen p-6 bg-green-800">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          ...
          return <Card key={plan.name} className={`card-senior relative ${plan.popular ? 'ring-2 ring-primary shadow-[var(--shadow-medium)]' : ''}`}>
                ...
                <CardContent className="pt-0">
                  ...
                  <Button onClick={() => handleSubscribe(plan.name, plan.price, plan.link)} className={`w-full btn-senior ${plan.popular ? 'gradient-primary text-white' : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'}`} disabled={selectedPlan === plan.name}>
        ...
        <div className="text-center mt-8">
          <Button variant="outline" onClick={() => navigate(-1)} className="btn-senior">
            Voltar
          </Button>
        </div>
      </div>
